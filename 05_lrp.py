...
def apply_lrp_on_vgg16(model, image):
    ...
    activations[-1] = torch.tensor(one_hot_output).unsqueeze(0).float().to(device)
...
# %%
# %% ==========================================
# Task 3: Second XAI Method - Grad-CAM Implementation
# ==========================================
...
original_image_np = input_image.permute(1, 2, 0).detach().cpu().numpy()

for module in model.modules():
    if isinstance(module, torch.nn.ReLU):
        module.inplace = False

grad_cam_heatmap = generate_gradcam(model, input_image, target_layer)


import cv2

grad_cam_resized = cv2.resize(grad_cam_heatmap, (255, 255))
...
plt.subplot(1, 3, 2)
plt.imshow(image_relevances[:, :, 0], cmap="seismic")
plt.title("XAI Method 1: LRP\n(Pixel-wise Relevance)")
plt.axis('off')


plt.subplot(1, 3, 3)
plt.imshow(original_image_np)
plt.imshow(grad_cam_resized, alpha=0.5, cmap='jet')
plt.title("XAI Method 2: Grad-CAM\n(Region Focus)")
plt.axis('off')

plt.tight_layout()
print(plt.show())

print("Grad-CAM complete。Please contrast LRP and Grad-CAM。")
